      ...
      ExecStart = "${pkgs.writeShellScript "watch-gnome-theme" ''
        ...
        dconf watch "/org/gnome/desktop/interface/color-scheme" | while read value; do
          if [[ "$value" == "'prefer-dark'" ]]; then
            home-manager switch --flake ~/pet/nixos/home/dark && nvr -c 'AirlineTheme dark' --nostart -s && nvr -c 'set background=dark' --nostart -s
          fi
          if [[ "$value" == "'default'" ]]; then
            home-manager switch --flake ~/pet/nixos/home/light && nvr -c 'AirlineTheme sol' --nostart -s && nvr -c 'set background=light' --nostart -s
          fi
        done
      ''}";
    };

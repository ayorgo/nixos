...
syn keyword pythonKeyword       break continue return pass yield with assert as
syn keyword pythonStatement     del global lambda
syn keyword pythonRaise         raise nextgroup=pythonExClass skipwhite
syn keyword pythonFuncDef       def nextgroup=pythonFunc skipwhite
...
if s:Python2Syntax()
    ...
    syn match   pythonFuncName    '[a-zA-Z_][a-zA-Z0-9_]*' display contained
else
    syn keyword pythonStatement   nonlocal
    syn match   pythonStatement   '\v\.@<!<await>'
    syn match   pythonFuncName    '\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*' display contained
    syn match   pythonStatement   '\<async\s\+def\>' nextgroup=pythonFuncName skipwhite
    syn match   pythonStatement   '\<async\s\+with\>'
    syn match   pythonStatement   '\<async\s\+for\>'
    syn cluster pythonExpression contains=pythonStatement,pythonKeyword,pythonRepeat,pythonConditional,pythonOperator,pythonLogicalOperator,pythonNumber,pythonHexNumber,pythonOctNumber,pythonBinNumber,pythonFloat,pythonString,pythonBytes,pythonBoolean,pythonNone,pythonSingleton,pythonBuiltinObj,pythonBuiltinFunc,pythonBuiltinType
endif
...
syn keyword pythonTodo          TODO FIXME XXX HACK contained
...
if v:version >= 508 || !exists('did_python_syn_inits')
    ...
    HiLink pythonRaise            Keyword
    HiLink pythonRaiseFromStatement   Statement
    HiLink pythonImport           Include
    HiLink pythonFuncDef          Function
